nodeMover7c=function(layout.old, g, node_number, spaceMax){
  ...
  for (move in 1:gran.length) {
    ...
    if ( length(gran.members[[move]]) > 1 ) {
      ...
      boundResp <- indexLookUp2(gran.members[[move]], posit.move, layout.old, spaceMax)
      df.list <- boundResp[[1]]
      ...
      #for ( i in 1:length(df.list) ){ df.truefalse[i] = layoutOverlapFinder(layout.old, df.list[[i]] ) }
