...
while(total_time <= RunLength){ 
  ...
  #this defines whether to move, form a bond or break a bond
  whatToDo <- sample(1:3, 1, prob = c(sigma, P.int.on, P.int.off) )
  
  if (whatToDo == 1){
    #this function moves nodes once per time step, assuming whatToDo = 1
    layout.old = nodeMover10(layout.old, g, node_number, spaceMax)
   }
  
  if (whatToDo == 2){
    #this randomly selects which nearby node to form an edge to
    ...
    optionsToBind <- `if`( is.null( nrow(optionsToBind) ), optionsToBind , optionsToBind[sample(1:nrow(optionsToBind), 1),] )
     ...
     #this is a clean up script from an earlier version and may not be needed anymore. 
  ...
  if (whatToDo == 3){
    #this randomly deletes edge. Edges are deleted probabilistically, selecting nodes with single edges preferentially to those with more than 1 edge
  ...
  #plot graph at each step. This will make the run time much longer, but useful to visualize sometimes 
  #plot(g, layout = layout.old,
  #     rescale = F, axes=TRUE, xlim=c(0,spaceMax), ylim=c(0,spaceMax), asp = 0 )
  
  #perform walktrap to capture granules
  gran_pop[[total_time]] = walktrap.community(g)
  #captures mRNPs (anything with 1 or more edges)
  mRNP_pop[[total_time]] = which(degree(g) >= 1)
  names(mRNP_pop) <- 'mRNP'
  #capture mRNAs (anything with 0 edges)
  mRNA_pop[[total_time]] = which(degree(g) < 1)
  names(mRNA_pop) <- 'mRNA'
  
  #capture graph and layout
  layout_list[[total_time]] = layout.old
  g_list[[total_time]] <- g
  
  #print time step
  print(total_time)
  #move total_time by one
